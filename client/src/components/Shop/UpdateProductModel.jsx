...
const UpdateProductModel = ({ product, onEditComplete }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${server}/product/edit/${product._id}`,
        newForm,
        config
      );
      ...
      if (res.status === 201) {
        ...
        clearForm();
        onEditComplete({ product: res.data.product });
      }
    } catch (err) {
    ...
    <>
      <div className=" bg-white h-full shadow rounded-[4px] p-10">
        ...
        <p className="text-[#000000a6] text-center mt-2">
          Enter product information that you need to edit, ensure that you enter
          correct information.
        </p>
        {/* create product form */}
        <form
